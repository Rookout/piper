...
| piper.argoWorkflows.server.token | string | `nil` | This will create a secret name piper-argo-token and use the field 'token' |
| piper.argoWorkflows.server.tokenExistingSecret | string | `nil` | Refrence to existing token with 'token' key. |
...
| piper.gitProvider.organization.webhook.orgLevel | bool | `false` | Whether config webhook on org level |
| piper.gitProvider.organization.webhook.repoList | list | `[]` | Used of orgLevel=false, to configure webhook for each of the repos provided. |
